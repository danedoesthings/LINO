...
def pipeline_validate_stage(code, stage_name, strict=False):
...
def run_transformer_isolated(transformer, code):
    ...
    try:
        ...
        pipeline_validate_stage(after_str if after_str else "", f"transform:{transformer.name}", strict=True)
        return after
    except LinoError:
        raise
    except Exception as e:
