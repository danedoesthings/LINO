...
if not newproxy then
    newproxy = function(addmeta) local p = {}; if addmeta then setmetatable(p, {}) end; return p end
end
if not unpack then
    unpack = table.unpack or function(t, i, j) j = j or #t; i = i or 1; if i > j then return end; return t[i], unpack(t, i + 1, j) end
end
...
def _capture_via_subprocess(source, timeout=120):
    src_path = None
    harness_path = None
    try:
...
def _lupa_decode_wearedevs(source):
    trace = []
    out, msg = _capture_via_subprocess(source, timeout=120)
    trace.append(f"subprocess: {msg}")
    if out and out.strip():
        return out.strip(), trace
    return None, trace

def _lupa_run(source, timeout_seconds=30):
    return _capture_via_subprocess(source, timeout_seconds)
