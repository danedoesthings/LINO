...
# Common single-letter VM variable names mapped to readable names
VM_SINGLE_LETTERS: dict[str, str] = {
    ...
    'k': 'keyTbl', 'x': 'idxVar', 'v': 'value', 's': 'strPool',
    'd': 'data', 'c': 'code', 'f': 'func', 'g': 'global',
    'm': 'meta', 'p': 'param', 't': 'table', 'n': 'num',
    'a': 'arg', 'b': 'buf', 'i': 'index', 'j': 'jindex',
}

# Semantic renames for common Lua function patterns
SEMANTIC_RENAMES: list[tuple[str, str]] = [
    (r'\bloadstring\b', 'dynLoader'),
    (r'\bload\b(?!\w)', 'dynLoader'),
    (r'\bpcall\b', 'safecall'),
    (r'\bxpcall\b', 'safecallEx'),
    (r'\bsetmetatable\b', 'setMeta'),
    (r'\bgetmetatable\b', 'getMeta'),
    (r'\brawset\b', 'rawSet'),
    (r'\brawget\b', 'rawGet'),
    (r'\bEncryptedStrings\b', 'strTable'),
    (r'\bGetString\b', 'strGet'),
    (r'\bVirtualStack\b', 'vmStack'),
    (r'\bInstructionTable\b', 'instrTbl'),
    (r'\bAllocSlot\b', 'allocSlot'),
    (r'\bPackArgs\b', 'packArgs'),
    (r'\bCallEnv\b', 'callEnv'),
    (r'\bAlphabetMap\b', 'alphaMap'),
    (r'\bcharFunc\b', 'charFn'),
    (r'\bFuncWrap\b', 'funcWrap'),
    (r'\bShuffleTable\b', 'shuffleTbl'),
    (r'\bTokenMap\b', 'tokenMap'),
    (r'\bCleanupRef\b', 'cleanRef'),
]


def is_probably_text(data) -> bool:
    """Check if binary data is likely text."""
    if not data:
        return False
