...
def extract_strings(source: str):
    # Added Y and generic patterns to catch WeAreDevs variants
    patterns = [
...
class StringTableDecoder:
    ...
    def _decode(self):
        ...
        for s in raw:
            ...
            if decoded_str is None:
                # CRITICAL FIX: if it looks like base64 but failed to decode, discard it
                if self.alphabet and len(s) >= 4 and all(c in self.alphabet or c == '=' for c in s):
                    decoded_str = ''
                elif is_printable(s) and len(s) > 1:
                    decoded_str = s
                else:
                    decoded_str = ''
    ...
    def get_source(self):
        ...
        # First pass: actual Lua source
        for s in self.strings:
            if is_lua_source(s):
                return s
        # Second pass: code-like fragments (NOT raw encoded strings)
        ...
        for s in self.strings:
            ...
            if is_printable(s) and any(kw in s for kw in code_keywords):
                # Must look like code, not base64
