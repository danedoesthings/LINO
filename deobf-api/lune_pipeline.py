...
def run_lune_darklua_pipeline(source: str) -> str:
    ...
    try:
        ...
        for index, raw_string in string_lookup.items():
            ...
            source_code = source_code.replace(f'R["{index}"]', f'"{safe_string}"')
        folded = fold_constants(source_code)
        ...
        if getter_match:
            ...
            offset_const = safe_eval_int(getter_match.group(2))
            if offset_const is not None:
                def repl(m):
                    idx = safe_eval_int(m.group(1))
                    if idx is not None:
                        str_idx = str(idx + offset_const)
                        if str_idx in string_lookup:
                            safe_str = string_lookup[str_idx].replace('\\', '\\\\').replace('"', '\\"')
                            return f'"{safe_str}"'
                    return m.group(0)
                source_code = re.sub(rf'{getter_name}\s*\(\s*(-?\d+(?:[+\-]\d+)*)\s*\)', repl, source_code)
