...
SUCCESS_METHODS = ('prometheus_decode',)
...
async def run_deobf(raw_bytes, filename):
    ...
    try:
        ...
        return {
            'embed': discord.Embed(title='API Timeout', description='API did not respond within 180 seconds', color=0xe74c3c),
            'files': [],
        }
    ...
    if 'error' in data:
        return {
            'embed': discord.Embed(title='Deobfuscation Failed', description=data['error'][:1500], color=0xe74c3c),
            'files': [],
        }
