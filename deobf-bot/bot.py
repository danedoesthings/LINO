...
async def call_api_direct(source_b64):
    """Call the deobfuscation API."""
    async with httpx.AsyncClient(timeout=180) as client:
        response = await client.post(
            f'{API_URL}/deobf/direct',
...
async def run_deobf(raw_bytes, filename):
    ...
    try:
        ...
        return {
            'embed': discord.Embed(
                title='API Timeout',
                description='API did not respond within 180 seconds',
                color=0xe74c3c
            ),
            'files': []
        }
    ...
    if data.get('status') != 'complete':
        ...
        return {
            'embed': discord.Embed(
                ...
                description=str(error_msg)[:1500],
                color=0xe74c3c
            ),
            'files': []
        }
...
async def prefix_deobf(ctx):
    """Deobfuscate a Lua file via prefix command."""
