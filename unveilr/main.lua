...
local Deferred = {} :: { func }
local IteratorTypes = {} :: { [string]: { any } }
local AliasToName = {
    int64 = "number",
    double = "number",
    int = "number"
...
local luteExists = (process.os == "windows" and fs.isFile("lute.exe")) or fs.isFile("bin/lute-linux") or fs.isFile("lute")
local hookOp = Settings.hookOp and luteExists
...
local function UseHookOp(Source : string, SourceFile : string?) : (boolean, string)
    ...
    if process.os == "windows" then
        luteBin = "lute.exe"
    elseif fs.isFile("bin/lute-linux") then
        luteBin = "bin/lute-linux"
    elseif fs.isFile("lute") then
        luteBin = "lute"
    else
        if FileName ~= SourceFile then
            pcall(fs.removeFile, FileName)
        end
        return false, "--errHookOp binary not available on this platform"
    end
    local hookOpModule
    if hookOp and fs.isFile("mods/hookOp.luau") then
        hookOpModule = "mods/hookOp.luau"
    elseif fs.isFile("mods/hookOpButNotReally.luau") then
        hookOpModule = "mods/hookOpButNotReally.luau"
    else
        if FileName ~= SourceFile then
            pcall(fs.removeFile, FileName)
        end
        return false, "--errHookOp module not found"
    end
    ...
    local execOk, POut = pcall(process.exec, luteBin, Args)
    if FileName ~= SourceFile then
        pcall(fs.removeFile, FileName)
    end
    if not execOk then
        pcall(fs.removeFile, OutPath)
        return false, "--errHookOp binary failed to execute: " .. tostring(POut)
    end
    ...
    if not s then
        local errMsg = (POut.stderr or ""):match("parsing failed:[^:]+: ([^\n]+)") or POut.stderr or "unknown error"
...
local function EnvLog(Source : (any) -> any, Nest : number) : AstExpression
    ...
    local function checktype(obj : any, expected : string, msg : string, allowNn : boolean?) : any
        if allowNn and not Internal[Variables[obj]] then return obj end
    ...
    local function Explore(Func : (any) -> any, Options : { [string]: any }, ...) : AstBlock
        ...
        local Gotten = {};
        if not Params and (Nest ~= 1 or not Options.as_pcall) then
    ...
    local function AppendCall(FuncName : string, ...) : (string, AstExpression)
        OpCountCheck();
        local Value = VarValues[FuncName]
        if Value and Value.kind == "index" then
            ...
            if ValueBase.kind == "variable" then
                ...
                if Name == FirstArg and (MethodInfo and MethodInfo.kind == "index") then
                    local Key = MethodInfo.data.key
                    local KeyVar = Key.data.value
                    if Key.kind == "variable" then
                        KeyVar = VarValues[MethodInfo.data.key.data.name].data.value
    ...
    function Spy(Path : string, Value : any, Info: any) : any
        ...
        if Reversed then return Reversed end
        if not Metatable then
            ...
            function Metatable.__newindex(Self, Key, Value)
                ...
                local Cached = (Internal[Path] or {{}})[1] :: { [string]: any };
                if rawequal(Cached, Env) then Cached[Key] = Value return end
                local Type = Typeof(Cached)
            ...
            function Metatable.__index(Self, Key : string)
                ...
                if rawequal(Cached, Env) then return Cached[Key] end
            ...
            function GetValue(x, Symbol, y, Var)
                ...
                if not InstantlyProcess[Symbol] then
                    if Symbol == ".." then
                        ...
                        if VarX or VarY then
                            if (not xLune and not yLune) or ((VarX and not xLune) or (VarY and not yLune)) then
                                if not LuaTypes[TypeX] and not LuaTypes[TypeY] then
    ...
    if hookOp then
        ...
        local function Eq(Symbol)
            ...
            return function(x, y)
                ...
                if (not A and not B) or (IgnoreInEq[A] or IgnoreInEq[B]) then return eq(x, y) end
        ...
        local CompVars, CheckCount, CompIgnore, AlreadyChecked = {}, {}, { table = true, string = true }, {};
        local IfBranchInfo = {} :: { [any]: { number } }
        ...
        local function Compare(Symbol)
            ...
            return function(x, y, isYFunc, ...)
                ...
                if ShouldReturn then
                    ...
                    if (a or b) and TypeX ~= "number" and TypeX ~= "Instance" then
                        ...
                        if not IsPrometheus then
                            ...
                            if X == false then
                                ...
                                if not Val or Val.kind ~= "unary" then
                                    return X
                                end
                            end
                            ...
                            Var = DefineVar(X and ToAST_X or Ast.UnaryOp(ToAST_X, "not"))
                        end
                ...
                if (not a and not b) or (IsFunction or CompIgnore[a]) then
                    return GetValue(X, y)
                end
                local Var;
                ...
                elseif not IsPrometheus then
                    if IsFunction and iscclosure(x) then
                        return GetValue(x, y);
                    end
                    Var = Def(x, Symbol, y, _y, isYFunc);
                end
        ...
        local LastCondition, LastConditionV2 = nil, nil;
        function SecureEnv.GET(text : any, ...)
            ...
            then
                local PreviousString = text:sub(1, -2)
        ...
        function SecureEnv.CALL(func : func, line : number, col : number, ...) : any
            ...
            if Type ~= "function" then
                ...
                if var and LuaTypes[Typeof(func)] then
                    return __call(func, ...);
                end
            end
        ...
        function SecureEnv.CHECKIF(Condition : any, id : number, elseId : number) : any
            ...
            if Var then
                ...
                OpCountCheck();
                insert(WaitingIfs, { _Condition, #AST.data.statements, not WillRun and HasElse, HasElse, IfId })
                return WillRun;
            end
            return Condition
        end
        function SecureEnv.CHECKIFEND(id : number)
            ...
            if Oldest[2] == LengthNow then return end
            ...
            if not TblVar and tbl then
                ...
                if ShouldSpy then
                    ...
                    if typeof(mt) == "table" then
                        ...
                        if info and info.is_property or Settings.discord then
        ...
        local HowManyChecks = 6;
        local function NumCompare(Symbol)
            ...
            return function(IsMarCute, YesSheIs)
                ...
                if not Success then
                    ...
                    if VarB and IsMarNumber then
                        Val = IsLessThan or IsLessOrEqual
                    elseif VarA and TypeShi == "number" then
    ...
    local function TypeFunction(Name)
        return Function(function(Value : any) : string
            ...
            Type = Name == "typeof" and Type == "vector" and "Vector3" or Type;
    ...
    local function IsWeird(Key) : boolean
        if Key == "" or typeof(Key) ~= "string" or tonumber(sub(Key, 1, 1)) then return true end
    ...
    if Nest == 0 then
        ...
        local function HttpGet(Self : VariableData, Url : string, Sync: boolean?)
            ...
            if Gets <= AllowedGetRequests and typeof(Url) == "string" then
        ...
        local function UnsafeFindFirstChild(Indexed : string, SpiedObject : Spied, Key : string)
            local RealInstance = SpiedToInstance[SpiedObject]
            local Kids = {}
            if not RealInstance then return end
            if RealInstance.ClassName == "DataModel" and Services[Key] then
                return CreateInstance(Indexed, Key, true);
            end
            for _, v in next, Kids do
                if SpiedToInstance[v].Name == Key then
                    return SafeValue(Indexed, v);
                end
            end
        end
        function CreateInstance(Variable : string, Class : string?, EnableCache : boolean?, FromInstanceNew : boolean?)
            ...
            local Mt = {
                __index = function(Self, Key)
                    ...
                    if IsLowerCase(Prefix:byte()) then
                    ...
                    local Child = UnsafeFindFirstChild(Indexed, Metatable, Key)
                    if Child then
                        return Child
                    end
                    if Class and Class ~= "Instance" then
        ...
        MethodsHooks["*"].WaitForChild = function(Self : VariableData, childName : string, timeOut : number?)
            ...
            local Instance = UnsafeFindFirstChild(Self.var, Self.self, childName)
            if Instance then return Instance end
            Instance = CreateInstance(Self.var)
    ...
    SecureEnv.string = HookLibrary("string", {
        ...
        gmatch = function(...)
            ...
            if DoLog or DontRun then
                ...
                local Success, Iterator = pcall(string.gmatch, unpack(args))
    ...
    do
        ...
        local Libraries = {
            ...
            "RaycastParams", "PhysicalProperties", "Random", "DateTime",
    ...
    SecureEnv.require = function(path : any)
        ...
        if Type == "string" then
            ...
            error(Unable)
        elseif Type ~= "number" and (Type ~= "Instance") then
    ...
    SecureEnv.pcall = function(func, ...)
        ...
        if not ReturnValues[1] then
            ...
            if Type == "error" or Type == "string" then
                ...
                local RandomString = Error:find("attempt to index nil with 'randomString'", 0, true)
                if not HasArithmetic and not RandomString then
                    ReturnValues[2] = HideErrors(Error):match("[^\n]+")
                else
                    local Line = (HasArithmetic and Error:find("attempt to perform arithmetic (unm)", 0, true)) and "1" or "%2"
                    ReturnValues[2] = Error:gsub(UserRegex, `[string "{TraceId}"]:{Line}`)
                end
                ReturnValues[2] = ReturnValues[2]:gsub("luau%.load%(%.%.%.%)", TraceId)
                if startswith(ReturnValues[2], "terrain:") then
    ...
    function GetInfo(i : number | thread) : { [any]: any }?
        ...
        local True = TrueEnv[Variables[i]]
...
local function TimedDump(File : string, RawContent : boolean?)
    ...
    if A == 6 and B == 3 and C == 159 and D == 1 then
        IsBytecode = true
    end
    ...
    if Settings.version and not RawContent then
        fs.removeFile(File);
    end
